...
<p align="center">
  <img src="docs/media/hero.png" width="900" alt="Ventic home screen">
  <br/>
...
- **Your library stays yours.** History, progress, favourites and the watchlist live in this
  device's storage. No Ventic account, no Ventic server, one-file backup, optional Trakt sync.
...
<sub><b>Appearance</b> &middot; 27 themes, accent colours, poster size, app scale and a global CSS
box.</sub>
</td>
</tr>
</table>
...
| **Favourites** / **Watchlist** / **History** | The three local lists, each optionally mirrored to Trakt |
...
27 themes, dark and light — Ventic Dark, Midnight (OLED), Carbon, Monochrome, Nord, Tokyo Night,
Mocha, Dracula, Rosé Pine, Gruvbox, Monokai, Solarized, Ember, Forest, Abyss, Crimson, Amethyst,
Ventic Light, Paper, Latte, Frost, Mint and more. Each takes an accent colour, and every
foreground/background pair's contrast is asserted in `bun run check:theme` rather than eyeballed.

Poster size, grid or list layout, app scale, backdrop behaviour and a global CSS box are all in
*Settings → Appearance*.
...
in this device's `localStorage`. There is no Ventic account and no Ventic server. Two things stop
that meaning "one cleared webview from gone":

**A backup file.** *Settings → Account → Save a backup* writes `ventic-backup.json` into your
documents folder (a browser session downloads it instead), and *Restore…* reads it back after
showing what it holds. It carries your sources too, so restoring changes what the app searches —
which is why it asks first. It deliberately does **not** carry the Trakt token: a backup is
something you copy onto a stick and mail to yourself, and a credential in one can't be taken back.

**Trakt**, if you want it.

### Trakt

Optional, off unless you connect it, and everything works without it. Connected, Ventic reports
what it is playing as it plays and pulls back what you watched elsewhere — so a show started on a
laptop carries on from the same place on the TV, and an episode seen on the TV stops coming up on
the laptop.

- **Signing in is a device code.** Trakt shows six characters, you type them on `trakt.tv/activate`
  from whatever device is handy. No password is typed into Ventic, no redirect URI, no loopback
  server — the only flow that works the same on a TV as on a desktop.
- **Out:** playback is scrobbled (start on play, pause when paused, stop when the player is put
  down — past ~80% Trakt files it as watched, below that as a resume point). Watched ticks go to
  your history; the heart and the bookmark go to your Trakt favourites and watchlist. Marking a
  whole season is one request, not one per episode.
- **In**, on connect and once per session after: watched marks, resume points, favourites and the
  watchlist. A record made here always wins if it is newer, and something finished here is never
  walked back to part-watched.
- **Trakt caps its lists; Ventic doesn't.** A free account holds 250 watchlist items. Once a list
  here is past whatever your account allows, the extra titles stay on this device and the page
  says so. Nothing is refused, nothing is dropped, and the cap is read from your account rather
  than assumed.
- Everything is keyed by **TMDB id**, so nothing is matched on a name.

Sources, downloads and settings are never sent anywhere. Only watch state is.
...
- **No account and no server.** There is nothing to sign into except Trakt, if you choose to.
- **Credentials stay on the device.** The Trakt token lives in local storage and is deliberately
  left out of the backup file. Debrid keys, if you use one, live inside the source URL you added.
...
| **Trakt** (`api.trakt.tv`) | Only if you connect an account, and only watch state. Never your sources, downloads or settings. Disconnecting stops it immediately and forgets the token. |
...
| `TRAKT_ID` / `TRAKT_SECRET` | no | Enables the Trakt section. Without them, *Settings → Account* says there is no Trakt to connect to. |
| `TAURI_DEV_HOST` | no | The address an Android device reaches your dev server on. |

Builds carry their own Trakt credentials, so a build from source needs its own — register an app
at [trakt.tv/oauth/applications][trakt-app] with `urn:ietf:wg:oauth:2.0:oob` as the redirect URI.
...
app/
  ...
  stores/          library, downloads, settings, trakt, ui (Pinia)
  utils/           torrents, subtitles, tmdb, trakt, backup, htmlvideo, dpad
  plugins/         dpad, deep links, drawer swipe
  theme/           palette generation and the 27 presets
src-tauri/
...
bun run check:trakt             # what wins when two devices disagree about an episode
...
- **[Trakt](https://trakt.tv/)** — optional watch-state sync.
...
[badge-version]: https://img.shields.io/badge/version-0.1.0-FF5555?style=for-the-badge&labelColor=1a1a1a
