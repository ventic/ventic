...
import { continuing, finished, fraction, nextEpisode, parseKey, placeholder, playedTitles, progressKey, remainingText, resumable, showEntries, slim, UNKNOWN_TITLE, watchedTitle } from '../app/utils/library'
...
// --- What may be favourited --------------------------------------------------
// Trakt takes a favourite only for something in your watched history, so the
// button is gated the same way rather than storing one that can never sync.

assert.ok(watchedTitle(stored, 'movie', 27205), 'played out')
assert.ok(!watchedTitle(stored, 'movie', 603), 'halfway through is not watched')
assert.ok(!watchedTitle(stored, 'movie', 999), 'never touched')
// One watched episode is what puts a show in that history — the whole run is
// not the bar, or a favourite would be unreachable until the finale.
assert.ok(watchedTitle(stored, 'tv', 1396))
assert.ok(!watchedTitle(stored, 'tv', 1399), '30 seconds of the pilot is not')
// The hand mark on the show itself, with no episode ever played.
assert.ok(watchedTitle({ 'tv:1399': entry(0, 0, 1, true) }, 'tv', 1399))
