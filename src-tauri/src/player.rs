...
// fully cross-platform; only the X11 child-window embedding here is Linux-specific
// (Windows would use a child HWND, macOS the libmpv render API, Android libmpv
// through JNI — see player_unsupported.rs).
// ----------------------------------------------------------------------------
...
/// Windows/macOS builds will get one) wins over whatever is on PATH; otherwise
/// we fall back to the system install, which is the normal case on Linux.
...
pub fn player_start(
	...
	// Short socket path — AF_UNIX paths are limited to ~108 bytes.
	let ipc = std::env::temp_dir().join(format!("ventic-mpv-{}.sock", std::process::id()));
	let _ = std::fs::remove_file(&ipc);
...
pub fn player_ipc(state: tauri::State<'_, PlayerState>, command: String) -> Result<String, String> {
	...
	let mut stream = std::os::unix::net::UnixStream::connect(&path)
		.map_err(|e| format!("mpv not ready: {e}"))?;
