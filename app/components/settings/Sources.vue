...
function add() {
  ...
  if (!value) {
    error.value = 'That doesn\'t look like a URL. Paste an addon link, or one starting with https://'
    return
  }
  if (settings.sources.includes(value)) {
    error.value = 'That source is already in the list.'
    return
  }
...
<template>
  <div class="flex flex-col gap-8">
    <settings-section
      title="Sources"
      hint="Ventic searches nothing by itself. A source is a URL you add here, pointing at a
        server that answers with things to play. What a source offers, and whether you have
        the right to play it, is between you and whoever runs it."
    >
      ...
      <v-alert variant="tonal" density="comfortable" rounded="lg" class="text-body-medium">
        ...
        Ventic speaks the <strong>Stremio addon protocol</strong>. Any Stremio addon URL works
        here — paste the <code>stremio://</code> link or the <code>manifest.json</code> address
        ...
        <v-list-item v-for="value in settings.sources" :key="value" :title="value">
          <template #append>
            <v-btn icon size="small" variant="text" color="on-surface" @click="remove(value)">
              ...
              <v-tooltip activator="parent" text="Remove this source" />
            </v-btn>
          </template>
        </v-list-item>
      </v-list>
      ...
      <div v-else class="flex flex-col items-center gap-2 rounded-lg bg-surface-container/40 px-4 py-10 text-center">
        ...
        <p class="text-body-medium opacity-70">
          No sources yet. Until you add one, the app plays what you already have — anything
          in Downloads, and any magnet or torrent file you open yourself.
        </p>
      </div>
    ...
    <settings-section
      title="Adding by link"
      hint="A page can offer a ventic:// link that opens the app with a source ready to add.
        The app always asks first — a link can never change what Ventic searches on its own."
    >
      <v-switch
        ...
        density="comfortable"
        hide-details
        label="Also handle stremio:// links"
        @update:model-value="toggleStremio"
      />
      <p class="text-body-small opacity-70">
        Addon pages publish <code>stremio://</code> install links. Turning this on points them
        at Ventic. Leave it off if you also use Stremio — only one app can own the scheme, and
        this would take it.
      </p>
    </settings-section>
      ...
      <v-card rounded="xl">
        ...
        <v-card-text class="flex flex-col gap-3">
          <p class="text-body-medium">
            A link asked Ventic to start searching:
          </p>
          <code class="break-all rounded-lg bg-surface-container-high px-3 py-2 text-body-small">{{ pending }}</code>
          <p class="text-body-small opacity-70">
            Ventic will send it the title you're looking for and play what it hands back. Only
            add servers you trust — this one is not run by, or checked by, this app.
          </p>
        </v-card-text>
        <v-card-actions>
          <v-spacer />
    ...
    <settings-section
      title="What a source has to speak"
      hint="Any server implementing the Stremio addon protocol works — it is an open protocol
        with several independent implementations, and Ventic runs no code from a source, only
        reads its answer."
    >
      <p class="text-body-small opacity-70">
        Ventic asks a source for <code>/stream/movie/&lt;imdb-id&gt;.json</code>, or
        <code>/stream/series/&lt;imdb-id&gt;:&lt;season&gt;:&lt;episode&gt;.json</code>, and
        expects a <code>streams</code> array back. Add several and their results are merged,
        with duplicates dropped and earlier sources preferred.
      </p>
      <p class="text-body-small opacity-70">
        The project does not host, run, endorse or recommend any source, and does not
        distribute a list of them.
      </p>
    </settings-section>
  </div>
</template>
