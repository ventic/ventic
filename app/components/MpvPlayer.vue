...
import type { PlayEvent } from '~/utils/trakt'
import {
...
// ---------------------------------------------------------------------------
function saveProgress(event: PlayEvent = 'tick') {
  if (props.media && duration.value)
    library.record(props.media, props.season ?? 0, props.episode ?? 0, position.value, duration.value, event)
}

// Trakt is told about a play, not asked about one, so a pause it never hears
// leaves it showing this as still playing until the window closes. Resuming
// reports a plain tick, which is the `start` scrobble again.
watch(paused, p => saveProgress(p ? 'pause' : 'tick'))
