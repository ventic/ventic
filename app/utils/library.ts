...
/**
 * Whether a title counts as watched — Trakt takes a favourite only for things
 * already in your watched history and refuses the rest, so this is what gates
 * that button. A movie earns it on its own entry; a show on any watched
 * episode, which is exactly what puts a show in that history, or on the
 * hand mark against the show itself.
 */
export function watchedTitle(progress: Record<string, Progress>, type: MediaType, id: number | string) {
  if (progress[titleKey(type, id)]?.watched)
    return true
  return type === 'tv' && showEntries(progress, id).some(([, p]) => p.watched)
}
...
/**
 * A card for a title with no snapshot on this device: a Trakt sync whose TMDB
 * detail never landed, most often an id TMDB no longer answers for. Shown rather
 * than dropped — the entry is real and so is the watch state behind it, and a row
 * that silently isn't there reads as the app having lost it.
 *
